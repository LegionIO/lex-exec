...
RSpec.describe Legion::Extensions::Exec::Helpers::Checkpoint do
  ...
  before do
    Dir.chdir(tmpdir) do
      ...
      Open3.capture3('git', 'config', 'core.excludesFile', File::NULL)
      File.write('file.rb', 'original')
      Open3.capture3('git', 'add', '-f', 'file.rb')
  ...
  describe '.save' do
    it 'creates a checkpoint ref' do
      File.write(File.join(tmpdir, 'file.rb'), 'modified')
  ...
  describe '.restore' do
    it 'restores files to checkpoint state' do
      File.write(File.join(tmpdir, 'file.rb'), 'checkpoint-state')
      described_class.save(worktree_path: tmpdir, label: 'snap', task_id: task_id)

      File.write(File.join(tmpdir, 'file.rb'), 'later-state')

      result = described_class.restore(worktree_path: tmpdir, label: 'snap', task_id: task_id)
      expect(result[:success]).to be true
      expect(File.read(File.join(tmpdir, 'file.rb'))).to eq('checkpoint-state')
    end
  end
